...
public class DeviceInfoSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private static final String KEY_INTENSE_VERSION = "intense_version";
    ...
    public void onCreate(Bundle icicle) {
        ...
        setStringSummary(KEY_BUILD_NUMBER, "MMB29M");
        findPreference(KEY_BUILD_NUMBER).setEnabled(true);
        findPreference(KEY_KERNEL_VERSION).setSummary(getFormattedKernelVersion());
        setValueSummary(KEY_INTENSE_VERSION, "ro.intense.display.version");
        findPreference(KEY_INTENSE_VERSION).setEnabled(true);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference.getKey().equals(KEY_FIRMWARE_VERSION)) {
            ...
            sendFeedback();
        } else if (preference.getKey().equals(KEY_INTENSE_VERSION)) {
            System.arraycopy(mHits, 1, mHits, 0, mHits.length-1);
            mHits[mHits.length-1] = SystemClock.uptimeMillis();
            if (mHits[0] >= (SystemClock.uptimeMillis()-500)) {
                Intent intent = new Intent(Intent.ACTION_MAIN);
